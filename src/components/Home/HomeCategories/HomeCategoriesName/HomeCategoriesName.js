...
const HomeCategoriesName = ({ restaurantData, visibleId, visibleSubcategoryId, categoryIndex, setCategoryIndex }) => {
    ...
    <>
      <div className={styles.container}>
        <div ref={containerRef} className={styles.categoryCardContainer}>
          {
            restaurantData?.data?.categories?.map((category, index) => (
              category?.is_active &&
              <div onClick={() => handleScroll(category?.id, index)} key={category?.id} className={styles.categoryCard}>
                <img src={category?.icon} className={styles.categoryImg} loading='lazy' alt='category-image' width={300} height={300} />
      ...
      <div className={scrollActive ? styles.scrollCategoryCard : 'hidden'}>
        <div className={styles.categoryCard1}>
          {restaurantData?.data?.categories?.map((category, index) => (
            category?.is_active &&
            <div key={category?.id} onClick={() => handleScroll(category?.id, index)} className={`${styles.scrolLCategoryCard} ${visibleId == category?.id && styles.activeCategory}`}>
              <span>{category?.name}</span>
            </div>
          ))}
        </div>
        <div className={styles.categoryCard2}>
          {restaurantData?.data?.meals[categoryIndex]?.is_active &&
            restaurantData?.data?.meals[categoryIndex]?.subcategories?.map((subcategory) => (
              subcategory?.is_active &&
              <div key={subcategory?.id} onClick={() => handleSrollSubcategory(subcategory?.id)} id={`sub-${subcategory?.id}`} className={`${styles.subcategoryCard} ${visibleSubcategoryId == subcategory?.id && styles.activeSubcategory}`}>

...
import { GetHotelServices, HotelServiceDelete } from '@/services/api/dataApi'
...
const AdminCategoryTable = () => {
  ...
  const { data, isLoading } = useQuery(['service'], () => GetHotelServices())
console.log(data)
  const openServiceEdit = (serviceSlug) => {
    router.push(`?service=edit-technical-service&service-slug=${serviceSlug}&lang=az`)
  }
        ...
        <div>
          <div className="pl-10 mb-28">
            <div className="mb-12 flex w-[90%] justify-between gap-4">
              <button
                onClick={() => router.push(`?service=add-technical-service`)}
                className={`${styles.addBtnCategory} !w-[180px]`}
              >
                {t('Add Technical Service')}
              </button>
              <input
                type="text"
                  ...
                  {filteredService
                    ? filteredService.map((item, i) => (
                        <tr key={item.id} className={styles.tr}>
                          ...
                          <td className={styles.td}>{item.title_az}</td>
                          <td>
